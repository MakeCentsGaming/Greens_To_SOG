...
namespace Greens_To_SOG
{
    ...
    class clsDragNDrop
    {
      ...
      public static void ListDragNDrop(ListBox lb, MainWindow mw = null, bool firstfile = true)
      {
         if(mw!=null)
         {
            mw.AllowDrop = true;
            mw.Drop += listBox_Drop;
            mw.PreviewDragOver += listBox_DragOver;
         }
         //this.firstfile = firstfile;
         //this.tb = tb;
         lb.AllowDrop = true;
      ...
      private static void listBox_Drop(object sender, DragEventArgs e)
      {
         ...
         if (files != null && files.Length != 0)
         {
            ...
            {
               if(MainViewModel.Instance.DirList==null)
               {
                  MainViewModel.Instance.DirList = new System.Collections.ObjectModel.ObservableCollection<string>();
               }
